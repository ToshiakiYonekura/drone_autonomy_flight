#!/bin/bash
# All-in-one SITL altitude test

set -e

echo "========================================================================"
echo "COMPLETE SITL ALTITUDE TEST"
echo "========================================================================"
...
~/ardupilot/build/sitl/bin/arducopter --model + --speedup 1 \
  ...
  > /tmp/sitl_complete_test.log 2>&1 &

SITL_PID=$!
echo "   SITL PID: $SITL_PID"

# Wait for SITL to start
echo "3. Waiting 8 seconds for SITL to start..."
sleep 8

# Check if SITL is still running
if ! kill -0 $SITL_PID 2>/dev/null; then
    echo "   ❌ SITL crashed! Check /tmp/sitl_complete_test.log"
    exit 1
fi
...
# Run the Python test
echo "4. Running altitude change test..."
echo ""
cd /home/yonetoshi27/autonomous_drone_sim

python3 test_actual_altitude_change.py << EOF
y
EOF
...
if [ $TEST_RESULT -eq 0 ]; then
    echo "✅✅✅ TEST PASSED! ✅✅✅"
    echo "Altitude control is working correctly in SITL!"
elif [ $TEST_RESULT -eq 1 ]; then
    echo "❌❌❌ TEST FAILED! ❌❌❌"
    echo "Altitude control has issues - check output above"
else
    echo "⚠️⚠️⚠️  TEST INCONCLUSIVE ⚠️⚠️⚠️"
    echo "Could not determine if altitude control works - check output above"
fi
echo "========================================================================"
